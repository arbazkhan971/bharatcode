...
name: builtin-skills
description:
  Use when creating a new builtin skill for BharatCode, editing an existing builtin
...
BharatCode embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `bharatcode://skills/` prefix (e.g., `bharatcode://skills/jq/SKILL.md`).
- The View tool resolves `bharatcode://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `bharatcode-config` | `builtin/bharatcode-config/` | BharatCode configuration help                   |
| `bharatcode-hooks`  | `builtin/bharatcode-hooks/`  | Authoring, configuring and debugging hooks |
