# BharatCode Development Guide

## Project Overview

BharatCode is a terminal-based AI coding assistant built in Go by
...
The module path is `github.com/arbazkhan971/bharatcode`.
...
internal/
  ...
  config/
    ...
    load.go                        bharatcode.json loading and validation
  ...
  hooks/                           Hook engine: runs user shell commands on hook events
    ...
    input.go                       Stdin payload builder, env vars, stdout parsing (BharatCode + Claude Code compat)
...
- **Context files**: BharatCode reads AGENTS.md, BHARATCODE.md, CLAUDE.md, GEMINI.md
...
- **Pub/sub**: `internal/pubsub` for decoupled communication between agent,
  UI, and services.
- **Hooks**: User-defined shell commands in `bharatcode.json` that fire before
