...
the agent lifecycle, allowing you to both build on top of BharatCode, customize
...
- BharatCode ships with a builtin `bharatcode-hook` skill write, edit, and configure
  hooks; just tell BharatCode how to configure BharatCode
- BharatCode currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `bharatcode.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/bharatcode/bharatcode.json`), use an absolute path instead.
...
# ($BHARATCODE_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$BHARATCODE_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through BharatCode's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), BharatCode falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `BHARATCODE=1`, `AGENT=bharatcode`, and
  `AI_AGENT=bharatcode` on top of the `BHARATCODE_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, BharatCode cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, BharatCode abandons it, logs a
...
Hooks can be added to your `bharatcode.json` (or `.bharatcode.json`) at both the global
...
> work fine in project-level `bharatcode.json` because the project root is also
> your working directory. For **global** config (`~/.config/bharatcode/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/bharatcode/bharatcode.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/bharatcode/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through BharatCode's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, BharatCode treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as BharatCode will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `BHARATCODE`                      | Always `1` when running under BharatCode.           |
| `AGENT`                      | Always `bharatcode`.                                |
| `AI_AGENT`                   | Always `bharatcode`.                                |
| `BHARATCODE_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `BHARATCODE_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `BHARATCODE_SESSION_ID`           | Current session ID.                            |
| `BHARATCODE_CWD`                  | Working directory.                             |
| `BHARATCODE_PROJECT_DIR`          | Project root directory.                        |
| `BHARATCODE_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `BHARATCODE_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `BHARATCODE`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under BharatCode?" the same way in
either context.
...
Hooks communicate back to BharatCode via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `bharatcode.json`.
...
If a hook exceeds its timeout, BharatCode cancels its context and treats the
...
if echo "$BHARATCODE_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells BharatCode to pre-approve the call:
...
case "$BHARATCODE_TOOL_INPUT_COMMAND" in
...
if [[ "$BHARATCODE_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $BHARATCODE_TOOL_NAME\" >> ./tools.log" }
```
...
BharatCode hooks are broadly compatible with [Claude Code
...
hooks run under BharatCode unchanged. This document covers the BharatCode-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: BharatCode treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, BharatCode pre-approves the tool call and skips the permission
