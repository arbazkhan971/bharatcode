// Package agent is the core orchestration layer for BharatCode AI agents.
//
...
import (
	...
	"github.com/arbazkhan971/bharatcode/internal/agent/tools"
...
var userAgent = fmt.Sprintf("BharatCode/%s (https://bharatcode.dev)", version.Version)
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `bharatcode run` against a
	...
	// event, so non-interactive clients (e.g. `bharatcode run`) don't
...
// RunID, e.g. `bharatcode run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `bharatcode run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `bharatcode run`) are not left
...
// caller waiting on RunComplete (e.g. `bharatcode run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil {
		...
		if a.canceledBySeq(call.SessionID, call.Accepted.seq) {
			...
			// RunComplete for this RunID (e.g. `bharatcode run`, which
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "bharatcode auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("BHARATCODE_DISABLE_ANTHROPIC_CACHE")); t {
