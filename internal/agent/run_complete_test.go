...
// event into a closure nobody reads, and subscribers (`bharatcode run`)
...
// publish a RunComplete for its RunID, hanging a `bharatcode run` caller that
...
// than internal bookkeeping, which is the contract a `bharatcode run` caller
...
// bookkeeping slice, not the published event a `bharatcode run` caller awaits.
