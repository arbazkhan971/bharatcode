...
const BharatCodeLogsToolName = "bharatcode_logs"

//go:embed bharatcode_logs.md.tpl
var bharatcodeLogsDescriptionTmpl []byte

var bharatcodeLogsDescriptionTpl = template.Must(
	template.New("bharatcodeLogsDescription").
		Parse(string(bharatcodeLogsDescriptionTmpl)),
)

type bharatcodeLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func bharatcodeLogsDescription() string {
	return renderTemplate(bharatcodeLogsDescriptionTpl, bharatcodeLogsDescriptionData{
...
type BharatCodeLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewBharatCodeLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		BharatCodeLogsToolName,
		bharatcodeLogsDescription(),
		func(ctx context.Context, params BharatCodeLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runBharatCodeLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runBharatCodeLogs reads and formats the last N log entries from the given file.
func runBharatCodeLogs(logFile string, params BharatCodeLogsParams) string {
