...
// with reason=tool_use and used to terminate `bharatcode run` early via
...
// subscribers (e.g. a TUI watching workspace events while `bharatcode
...
// non-nil error from `bharatcode run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `bharatcode run` waits) exits cleanly rather
...
// queue scenario: `bharatcode run --continue` attaches to a session
