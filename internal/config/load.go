...
import (
	...
	"github.com/arbazkhan971/bharatcode/internal/discover"
	"github.com/arbazkhan971/bharatcode/internal/env"
	"github.com/arbazkhan971/bharatcode/internal/filepathext"
...
func PushPopBharatCodeEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "BHARATCODE_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "BHARATCODE_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("BHARATCODE_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if len(c.Options.GlobalContextPaths) == 0 {
		bharatcodeConfigDir := filepath.Dir(GlobalConfig())
		c.Options.GlobalContextPaths = []string{
			filepath.Join(bharatcodeConfigDir, "BHARATCODE.md"),
			filepath.Join(filepath.Dir(bharatcodeConfigDir), "AGENTS.md"),
		}
	}
	...
	if str, ok := os.LookupEnv("BHARATCODE_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("BHARATCODE_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated bharatcode.json placed above the project is never picked
...
func GlobalConfig() string {
	if bharatcodeGlobal := os.Getenv("BHARATCODE_GLOBAL_CONFIG"); bharatcodeGlobal != "" {
		return filepath.Join(bharatcodeGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalCacheDir() string {
	if bharatcodeCache := os.Getenv("BHARATCODE_CACHE_DIR"); bharatcodeCache != "" {
		return bharatcodeCache
	}
...
func GlobalConfigData() string {
	if bharatcodeData := os.Getenv("BHARATCODE_GLOBAL_DATA"); bharatcodeData != "" {
		return filepath.Join(bharatcodeData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/bharatcode/`
	// for linux and macOS, it should be in `$HOME/.local/share/bharatcode/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps BharatCode from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if bharatcodeSkills := os.Getenv("BHARATCODE_SKILLS_DIR"); bharatcodeSkills != "" {
		return []string{bharatcodeSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/bharatcode`.
...
// ProjectSkillsDir returns the default project directories for which BharatCode
