...
// already in use by another bharatcode process.
var ErrDataDirLocked = errors.New("data directory already in use by another bharatcode process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to bharatcode.db so users can `ls` and find it.
const dataDirLockFile = "bharatcode.lock"
...
// {dataDir}/bharatcode.lock. If the lock is already held by another
...
// BHARATCODE_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("BHARATCODE_SKIP_DATADIR_LOCK"))
	return v
}
