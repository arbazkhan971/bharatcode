...
func TestBharatCodeIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .bharatcodeignore file that ignores .log files
	require.NoError(t, os.WriteFile(".bharatcodeignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .bharatcodeignore file
	bharatcodeignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".bharatcodeignore"), []byte(bharatcodeignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .bharatcodeignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .bharatcodeignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .bharatcodeignore in subdir that ignores normal_nested
	subBharatCodeignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".bharatcodeignore"), []byte(subBharatCodeignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .bharatcodeignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .bharatcodeignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .bharatcodeignore")
