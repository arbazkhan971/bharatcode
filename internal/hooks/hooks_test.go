...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["BHARATCODE_EVENT"])
	require.Equal(t, "bash", envMap["BHARATCODE_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["BHARATCODE_SESSION_ID"])
	require.Equal(t, "/work", envMap["BHARATCODE_CWD"])
	require.Equal(t, "/project", envMap["BHARATCODE_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["BHARATCODE_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["BHARATCODE_TOOL_INPUT_FILE_PATH"])

	// Shared BharatCode markers must be present so hook-authored scripts can
	// detect they're running under BharatCode the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["BHARATCODE"])
	require.Equal(t, "bharatcode", envMap["AGENT"])
	require.Equal(t, "bharatcode", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$BHARATCODE_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("bharatcode format still works", func(t *testing.T) {
		t.Parallel()
