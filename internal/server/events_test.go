...
// are what non-interactive clients (e.g. `bharatcode run`) rely on to
...
// through the SSE envelope. Without these correlators, `bharatcode run`
...
// "agent failed" (returns non-zero from `bharatcode run`) from "agent
