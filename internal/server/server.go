...
import (
	...
	"github.com/arbazkhan971/bharatcode/internal/backend"
	"github.com/arbazkhan971/bharatcode/internal/config"
	_ "github.com/arbazkhan971/bharatcode/internal/swagger"
...
// socketDir returns the directory used for the BharatCode Unix socket.
...
// returned by [socketDir] and is named bharatcode-<uid>.sock, falling
// back to bharatcode.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/bharatcode-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "bharatcode.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("bharatcode-%s.sock", usr.Uid)
	}
...
// Server represents a BharatCode server bound to a specific address.
type Server struct {
