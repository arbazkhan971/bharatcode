...
type RunOptions struct {
	...
	// the BharatCode process cwd — hooks and the bash tool have different
...
// BharatCode handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard BharatCode middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to BharatCode's process
...
// regardless of the caller's environment because BharatCode shells are never
...
//  1. builtins first (so BharatCode's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized BharatCode
