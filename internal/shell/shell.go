...
// BharatCodeEnvMarkers returns a fresh slice of the environment variables that
// BharatCode unconditionally sets on every shell it spawns — both the interactive
...
func BharatCodeEnvMarkers() []string {
	return []string{
		"BHARATCODE=1",
		"AGENT=bharatcode",
		"AI_AGENT=bharatcode",
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by BharatCode.
	env = append(env, BharatCodeEnvMarkers()...)
