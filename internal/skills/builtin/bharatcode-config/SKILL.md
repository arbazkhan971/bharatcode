---
name: bharatcode-config
description: Use when the user needs help configuring BharatCode — working with bharatcode.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing BharatCode behavior.
---

# BharatCode Configuration

BharatCode uses JSON configuration files with the following priority (highest to lowest):

1. `.bharatcode.json` (project-local, hidden)
2. `bharatcode.json` (project-local)
3. `$XDG_CONFIG_HOME/bharatcode/bharatcode.json` or `$HOME/.config/bharatcode/bharatcode.json` (global)
...
BharatCode runs selected string fields through an embedded bash-compatible
...
`bharatcode.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch BharatCode in a directory whose `bharatcode.json` you haven't
reviewed.
...
```json
{
  "options": {
    ...
    "disabled_skills": ["bharatcode-config"],
    "tui": {
      "compact_mode": false,
      "diff_mode": "unified",
      "transparent": false
...
> `.agents/skills`, `.bharatcode/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".bharatcode/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".bharatcode/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `BHARATCODE_EVENT` | Event name (e.g. `PreToolUse`) |
| `BHARATCODE_TOOL_NAME` | Name of the tool being called |
| `BHARATCODE_SESSION_ID` | Current session ID |
| `BHARATCODE_CWD` | Current working directory |
| `BHARATCODE_PROJECT_DIR` | Project root directory |
| `BHARATCODE_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `BHARATCODE_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
BharatCode also supports the Claude Code hook output format:
...
- `BHARATCODE_GLOBAL_CONFIG` - Override global config location
- `BHARATCODE_GLOBAL_DATA` - Override data directory location
- `BHARATCODE_SKILLS_DIR` - Override default skills directory
