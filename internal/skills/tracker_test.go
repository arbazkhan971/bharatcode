...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (bharatcode-config).
	activeSkills := []*Skill{
		{Name: "bharatcode-config", Description: "BharatCode config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("bharatcode-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via bharatcode://...).
	tracker.MarkLoaded("bharatcode-config")
	require.True(t, tracker.IsLoaded("bharatcode-config"))
