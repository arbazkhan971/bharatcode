...
Failure handling: on non-zero exit, output is never filtered — all error lines are preserved verbatim. A hard cap of 500 lines is applied with a `[N more lines truncated]` notice if exceeded.
