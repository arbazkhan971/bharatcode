...
The parent directory is created when needed, the file is written atomically, and the file tracker records a create or edit with before and after hashes. When an existing file is overwritten, the result also includes a compact unified diff of the changed lines so you can see exactly what changed. When a language server is configured for the file, any errors and warnings it reports for the written file are appended to the result — fix the errors before moving on.
