...
import (
	...
	"github.com/arbazkhan971/bharatcode/internal/agent/hyper"
	"github.com/arbazkhan971/bharatcode/internal/agent/notify"
	agenttools "github.com/arbazkhan971/bharatcode/internal/agent/tools"
	...
	"github.com/arbazkhan971/bharatcode/internal/commands"
	...
	"github.com/arbazkhan971/bharatcode/internal/home"
	"github.com/arbazkhan971/bharatcode/internal/message"
	"github.com/arbazkhan971/bharatcode/internal/permission"
	"github.com/arbazkhan971/bharatcode/internal/pubsub"
	"github.com/arbazkhan971/bharatcode/internal/session"
	"github.com/arbazkhan971/bharatcode/internal/skills"
	"github.com/arbazkhan971/bharatcode/internal/stringext"
	"github.com/arbazkhan971/bharatcode/internal/ui/anim"
	"github.com/arbazkhan971/bharatcode/internal/ui/attachments"
	"github.com/arbazkhan971/bharatcode/internal/ui/chat"
	"github.com/arbazkhan971/bharatcode/internal/ui/common"
	"github.com/arbazkhan971/bharatcode/internal/ui/completions"
	"github.com/arbazkhan971/bharatcode/internal/ui/dialog"
	fimage "github.com/arbazkhan971/bharatcode/internal/ui/image"
	"github.com/arbazkhan971/bharatcode/internal/ui/logo"
	"github.com/arbazkhan971/bharatcode/internal/ui/notification"
	"github.com/arbazkhan971/bharatcode/internal/ui/styles"
	"github.com/arbazkhan971/bharatcode/internal/ui/util"
...
type UI struct {
	...
	// goal holds the /goal autonomous outer-loop state. When goalActive is
	// true, each finished agent turn (handled in the TypeAgentFinished case)
	// auto-continues toward goalText until the agent signals completion or
	// blocked, or goalIter reaches goalMax. Cancelling clears goalActive.
	goalActive bool
	goalText   string
	goalIter   int
	goalMax    int
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case app.UpdateAvailableMsg:
		text := fmt.Sprintf("BharatCode update available: v%s → v%s.", msg.CurrentVersion, msg.LatestVersion)
		if msg.IsDevelopment {
			text = fmt.Sprintf("This is a development version of BharatCode. The latest version is v%s.", msg.LatestVersion)
		}
		ttl := 10 * time.Second
...
func (m *UI) handleDialogMsg(msg tea.Msg) tea.Cmd {
	...
	case dialog.ActionStartGoal:
		...
		m.dialog.CloseDialog(dialog.CommandsID)
		if msg.Goal != "" {
			cmds = append(cmds, m.startGoal(msg.Goal))
			break
		}
		// Palette entry carries no goal text; prime the editor for "/goal ".
	...
	case uiChat, uiLanding:
		...
		case uiFocusEditor:
			...
			case key.Matches(msg, m.keyMap.Editor.SendMessage):
				...
				// /goal <objective> starts the autonomous goal outer loop.
				if rest, ok := strings.CutPrefix(value, "/goal"); ok && (rest == "" || rest[0] == ' ') {
					goalText := strings.TrimSpace(rest)
					if goalText == "" {
						return util.ReportWarn("usage: /goal <objective>")
					}
					m.randomizePlaceholders()
					m.historyReset()
					return tea.Batch(m.startGoal(goalText), m.loadPromptHistory())
				}
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	if os.Getenv("BHARATCODE_UI_DEBUG") == "true" {
...
func (m *UI) View() tea.View {
	...
	v.WindowTitle = "bharatcode " + home.Short(m.com.Workspace.WorkingDir())
...
// startGoal activates the /goal autonomous outer loop for goalText, sends the
// first (kickoff) turn, and returns a command batch that also shows a status
// line. Subsequent turns are driven from the TypeAgentFinished handler in
// handleAgentNotification.
func (m *UI) startGoal(goalText string) tea.Cmd {
	m.goalActive = true
	m.goalText = goalText
	m.goalIter = 1
	m.goalMax = goal.DefaultMaxIterations
	return tea.Batch(
		m.sendMessage(goal.KickoffPrompt(goalText)),
		util.ReportInfo(fmt.Sprintf("🎯 goal iteration %d/%d", m.goalIter, m.goalMax)),
	)
}

// stopGoal clears the /goal outer-loop state and, if status is non-empty,
// returns a command that shows it.
func (m *UI) stopGoal(status string) tea.Cmd {
	m.goalActive = false
	m.goalText = ""
	m.goalIter = 0
	m.goalMax = 0
	...
	return util.ReportInfo(status)
}

// advanceGoal evaluates the just-finished assistant turn for an active /goal
// run and either stops goal mode (on completion, blocked, or max iterations)
// or sends the next continue turn.
func (m *UI) advanceGoal() tea.Cmd {
	text := m.lastAssistantText(m.session.ID)
	switch {
	case goal.IsComplete(text):
		return m.stopGoal(fmt.Sprintf("🎯 goal complete after %d/%d iterations", m.goalIter, m.goalMax))
	case goal.IsBlocked(text):
		return m.stopGoal(fmt.Sprintf("🎯 goal blocked at iteration %d/%d", m.goalIter, m.goalMax))
	case m.goalIter >= m.goalMax:
		return m.stopGoal(fmt.Sprintf("🎯 goal stopped: reached max %d iterations", m.goalMax))
	default:
		m.goalIter++
		return tea.Batch(
			m.sendMessage(goal.ContinuePrompt(m.goalText)),
			util.ReportInfo(fmt.Sprintf("🎯 goal iteration %d/%d", m.goalIter, m.goalMax)),
		)
	}
}
...
// sessionID, or "" if none/unavailable. The /goal outer loop reads this to
// detect the GOAL_COMPLETE / GOAL_BLOCKED sentinels. Mirrors the synchronous
// ListMessages call used by the loadSessionMsg handler.
func (m *UI) lastAssistantText(sessionID string) string {
...
func (m *UI) cancelAgent() tea.Cmd {
	...
	if m.isCanceling {
		...
		m.todoIsSpinning = false
		m.renderPills()
		// Cancelling also stops the /goal autonomous outer loop.
		if m.goalActive {
			return m.stopGoal("🎯 goal cancelled")
		}
		return nil
	}
...
func (m *UI) handleAgentNotification(n notify.Notification) tea.Cmd {
	...
	case notify.TypeAgentFinished:
		...
		// /goal autonomous outer loop: when this finished turn belongs to the
		// active goal session, evaluate the result and either continue toward
		// the goal or stop. The loop is event-driven: each turn's completion
		// fires the next sendMessage; the iter<max guard prevents runaway.
		if m.goalActive && m.hasSession() && n.SessionID == m.session.ID {
			if cmd := m.advanceGoal(); cmd != nil {
				cmds = append(cmds, cmd)
			}
		}
		return tea.Batch(cmds...)
...
func (m *UI) newSession() tea.Cmd {
	...
	// Starting a fresh session also stops any active /goal outer loop so it
	// does not bleed continuation turns into the new session.
	m.stopGoal("")
	m.session = nil
	m.sessionFiles = nil
...
// renderLogo renders the BharatCode logo with the given styles and dimensions.
